...
public class PaySlipPanel extends JPanel {
   ...
   * 現在表示対象の従業員オブジェクト。
   */
  private EmployeeImpl currentEmployee;
  /**
   ...
   */
  public void displayPaySlip(EmployeeImpl emp) {
  ...
  protected void paintComponent(Graphics g) {
    ...
    g2d.drawString(currentEmployee.getEmployee().name(), x + valueOffset, y);
    y += lineHeight;

    g2d.drawString("従業員ID:", x, y);
    g2d.drawString(currentEmployee.getEmployee().employeeId(), x + valueOffset, y);
    y += lineHeight;

    g2d.drawString("従業員種別:", x, y);
    g2d.drawString(currentEmployee.getEmployeeType(), x + valueOffset, y);
    y += lineHeight;
    ...
    // g2d.drawString(String.format("%.1f 時間", pte.getHoursWorked()), x +
    // valueOffset, y);
    // y += lineHeight;
    ...
    // g2d.drawString(formatCurrency(fte.calculateOvertimePay()), x + valueOffset,
    // y);
    // y += lineHeight;
    ...
    // g2d.drawString(formatCurrency(fte.getCommuteAllowance()), x + valueOffset,
    // y);
    // y += lineHeight;
    // }
    // }
